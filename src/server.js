...
app.use((req, res, next) => {
    ...
    // Add currency information to all views
...
app.get('/', async (req, res) => {
    try {
        ...
        const [gospelSongs] = await pool.execute(`
            ...
            FROM songs 
            JOIN users ON songs.user_id = users.id 
            WHERE songs.is_sold = FALSE AND songs.style = 'gospel'
        ...
        const [secularSongs] = await pool.execute(`
            ...
            FROM songs 
            JOIN users ON songs.user_id = users.id 
            WHERE songs.is_sold = FALSE AND songs.style = 'secular'
            ORDER BY songs.play_count DESC
            LIMIT 6
        `);
        ...
        const [stats] = await pool.execute(`
            SELECT 
                (SELECT COUNT(*) FROM songs WHERE is_sold = FALSE) as available_songs,
